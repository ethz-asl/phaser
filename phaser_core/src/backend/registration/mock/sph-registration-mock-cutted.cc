...
model::RegistrationResult SphRegistrationMockCutted::registerPointCloud(
  ...
  model::PointCloudPtr point_cloud = cutPointCloud(raw_cloud,
      min_pt.y + 1 , min_pt.y + 0.7 * dist, "y");
  model::PointCloudPtr syn_cloud = cutPointCloud(raw_cloud,
      min_pt.y + 0.55*dist , min_pt.y + dist, "y");
